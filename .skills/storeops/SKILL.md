...
description: >
  ...
  the storeops CLI (alias: st). Use this skill when the user wants to publish
...
- **Binary**: `storeops` or the alias `st`
...
If auth fails with exit code 2, verify credentials and re-run `st auth login`.
...
st apple metadata localizations list --version-id VER_ID

# Update each locale
for LOCALE in en-US ja de-DE fr-FR; do
  st apple metadata localizations update --version-id VER_ID \
...
st apple screenshots images upload --set-id SET_ID --file screenshot1.png
st apple screenshots images upload --set-id SET_ID --file screenshot2.png

# 4. Reorder if needed
st apple screenshots images reorder --set-id SET_ID \
...
st apple testflight testers list --group-id GROUP_ID
```
...
# Set price
st apple iap prices set --iap-id IAP_ID --price-point PRICE_POINT_ID

# Submit for review
st apple iap submit --iap-id IAP_ID

# Subscriptions follow a similar pattern
st apple subscriptions groups list --app-id APP_ID
st apple subscriptions items create --group-id GRP_ID \
...
# Respond to a review
st apple reviews respond --review-id REV_ID \
...
st apple pricing points --app-id APP_ID

# Set price
st apple pricing set --app-id APP_ID --price-point POINT_ID

# Manage availability
st apple availability get --app-id APP_ID
st apple availability territories --app-id APP_ID
...
2. Parse the JSON error: `st ... 2>&1 | jq '.errors'`
3. For auth errors, run `st auth status` then re-login if expired.
...
- Verify state after mutations: after `submit`, check `st apple versions list` to confirm status.
